...
- Desktop file drop: Dropping local files from Finder or another desktop application into the main terminal inserts each trustworthy absolute path as an `@path` reference without submitting it, then restores terminal focus. The webview `File.path` is canonical with decoded `file://` URI data as fallback; a bare filename is never substituted for a missing absolute path. Remote sessions reject local external drops, while workspace-tree drops retain their endpoint match guard.
